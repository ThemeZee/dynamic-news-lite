...
class Dynamic_News_Popular_Posts_Widget extends WP_Widget {

	var $excerpt_length = 10;

	function __construct() {
		$widget_ops = array('classname' => 'dynamicnews_popular_posts', 'description' => __('Show popular posts with post thumbnails.', 'dynamicnews') );
		$this->WP_Widget('dynamicnews_popular_posts', 'Popular Posts (Dynamic News)', $widget_ops);
	...
	function widget($args, $instance) {
		$cache = wp_cache_get('widget_dynamicnews_popular_posts', 'widget');
		...
		$r = new WP_Query( apply_filters( 'dynamicnews_widget_popular_posts_args', array( 'posts_per_page' => $number, 'orderby' => 'comment_count', 'order' => 'DESC', 'no_found_rows' => true, 'post_status' => 'publish', 'ignore_sticky_posts' => true ) ) );
			...
			<?php  while ($r->have_posts()) : $r->the_post(); ?>
				...
				if ( $postmeta == 1 ) : ?>
					<div class="widget-postmeta">
						...
						<span class="widget-comment"><a href="<?php the_permalink() ?>#comments"><?php comments_number(__('No comments', 'dynamicnews'),__('One comment','dynamicnews'),__('% comments','dynamicnews')); ?></a></span>
					</div>
				<?php endif; ?>
			</li>
			<?php endwhile; ?>
...
<?php
		...
		wp_cache_set('widget_dynamicnews_popular_posts', $cache, 'widget');
	}
	...
	function flush_widget_cache() {
		wp_cache_delete('widget_dynamicnews_popular_posts', 'widget');
	}
...
?>
		...
		<p><label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of posts to show:', 'dynamicnews'); ?></label>
		...
		<label for="<?php echo $this->get_field_id('thumbnails'); ?>"><?php _e('Show Post Thumbnails?', 'dynamicnews'); ?></label></p>

		<p><label for="<?php echo $this->get_field_id('excerpt_length'); ?>"><?php _e('Excerpt length in number of words:', 'dynamicnews'); ?></label>
		...
		<label for="<?php echo $this->get_field_id('postmeta'); ?>"><?php _e('Show Postmeta(Date, Comments)?', 'dynamicnews'); ?></label></p>
<?php
	}
}
register_widget('Dynamic_News_Popular_Posts_Widget');
?>