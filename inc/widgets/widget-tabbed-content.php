...
class Dynamic_News_Tabbed_Content_Widget extends WP_Widget {

	var $number = 7;
	var $thumbs = 0;

	function __construct() {
		$widget_ops = array('classname' => 'dynamicnews_tabbed_content', 'description' => __('Show recent posts with post thumbnails.', 'dynamicnews') );
		$control_ops = array('width' => 450, 'id_base' => 'dynamicnews_tabbed_content');
		$this->WP_Widget('dynamicnews_tabbed_content', 'Tabbed Content (Dynamic News)', $widget_ops, $control_ops);
...
<?php
	...
	function widget($args, $instance) {
		$cache = wp_cache_get('widget_dynamicnews_tabbed', 'widget');
	...
	<?php
		...
		wp_cache_set('widget_dynamicnews_tabbed', $cache, 'widget');
	}
	...
	function widget_content($tab) {
		...
		switch($tab) {
			...
			case 1: // Archives
				$content = '<ul>' . wp_get_archives(apply_filters('dynamicnews_widget_tabbed_archives_args', array('type' => 'monthly', 'show_post_count' => 1, 'echo' => 0))) . '</ul>';
			break;

			case 2: // Categories
				$cat_args = array('title_li' => '', 'orderby' => 'name', 'show_count' => 1, 'hierarchical' => false, 'echo' => 0);
				$content = '<ul>' . wp_list_categories(apply_filters('dynamicnews_widget_tabbed_categories_args', $cat_args)) . '</ul>';
			break;

			case 3: // Links
				$content = wp_list_bookmarks(apply_filters('dynamicnews_widget_tabbed_links_args', array(
			...
			case 4: // Meta
				...
				$content .= '<li><a href="'.get_bloginfo('rss2_url').'" title="'.esc_attr(__('Syndicate this site using RSS 2.0', 'dynamicnews')).'">'. __('Entries <abbr title="Really Simple Syndication">RSS</abbr>', 'dynamicnews').'</a></li>';
				$content .= '<li><a href="'.get_bloginfo('comments_rss2_url').'" title="'.esc_attr(__('The latest comments to all posts in RSS', 'dynamicnews')).'">'. __('Comments <abbr title="Really Simple Syndication">RSS</abbr>', 'dynamicnews').'</a></li>';
				$content .= '<li><a href="'.esc_url('http://wordpress.org/').'" title="'.esc_attr(__('Powered by WordPress, state-of-the-art semantic personal publishing platform.', 'dynamicnews')).'">'. __( 'WordPress.org', 'dynamicnews') .'</a></li>';
				$content .= wp_meta() . '</ul>';
			break;

			case 5: // Pages
				$content = '<ul>' . wp_list_pages( apply_filters('dynamicnews_widget_tabbed_pages_args', array('title_li' => '', 'echo' => 0) ) ) . '</ul>';
			break;

			case 6: // Popular Posts
				$posts = new WP_Query( apply_filters( 'dynamicnews_widget_tabbed_popular_posts_args', array( 'posts_per_page' => $this->number, 'orderby' => 'comment_count', 'order' => 'DESC', 'no_found_rows' => true, 'post_status' => 'publish', 'ignore_sticky_posts' => true ) ) );
				if ($posts->have_posts()) :
					$content = '<ul>';
			...
			case 7: // Recent Comments
				...
				$comments = get_comments( apply_filters( 'dynamicnews_widget_tabbed_comments_args', array( 'number' => $this->number, 'status' => 'approve', 'post_status' => 'publish' ) ) );
				...
				if ( $comments ) {
					foreach ( (array) $comments as $comment) {
						...
						$content .=  sprintf(_x('%1$s on %2$s', 'widgets', 'dynamicnews'), get_comment_author_link(), '<a href="' . esc_url( get_comment_link($comment->comment_ID) ) . '">' . get_the_title($comment->comment_post_ID) . '</a>') . '</li>';
					}
				}
				$content .= '</ul>';
			break;
			...
			case 8: // Recent Posts
				$posts = new WP_Query( apply_filters( 'dynamicnews_widget_tabbed_recent_posts_args', array( 'posts_per_page' => $this->number, 'no_found_rows' => true, 'post_status' => 'publish', 'ignore_sticky_posts' => true ) ) );
				if ($posts->have_posts()) :
					$content = '<ul>';
					while ($posts->have_posts()) : $posts->the_post();
			...
			case 9: // Tag Cloud
				...
				$content .= wp_tag_cloud( apply_filters('dynamicnews_widget_tabbed_tagcloud_args', array('taxonomy' => 'post_tag', 'echo' => false) ) );
				$content .= "</div>\n";
			break;
	...
	function flush_widget_cache() {
		wp_cache_delete('widget_dynamicnews_tabbed', 'widget');
	}

	function widget_select_options($tab) {
		$options = '<option ' . selected( $tab, 0 ) .' value="0"></option>
					<option ' . selected( $tab, 1 ) .' value="1">'. __('Archives', 'dynamicnews') . '</option>
					<option ' . selected( $tab, 2 ) .' value="2">'. __('Categories', 'dynamicnews') . '</option>
					<option ' . selected( $tab, 3 ) .' value="3">'. __('Links', 'dynamicnews') . '</option>
					<option ' . selected( $tab, 4 ) .' value="4">'. __('Meta', 'dynamicnews') . '</option>
					<option ' . selected( $tab, 5 ) .' value="5">'. __('Pages', 'dynamicnews') . '</option>
					<option ' . selected( $tab, 6 ) .' value="6">'. __('Popular Posts', 'dynamicnews') . '</option>
					<option ' . selected( $tab, 7 ) .' value="7">'. __('Recent Comments', 'dynamicnews') . '</option>
					<option ' . selected( $tab, 8 ) .' value="8">'. __('Recent Posts', 'dynamicnews') . '</option>
					<option ' . selected( $tab, 9 ) .' value="9">'. __('Tag Cloud', 'dynamicnews') . '</option>';
		return $options;
	}
...
?>
		...
		<div style="background: #f5f5f5; padding: 5px 10px; margin-bottom: 10px;">
			<p><label for="<?php echo $this->get_field_id('tab_one_content'); ?>"><strong><?php _e('Tab 1:', 'dynamicnews') ?></strong></label>
				...
				<label for="<?php echo $this->get_field_id('tab_one_title'); ?>"><?php _e('Title:', 'dynamicnews'); ?></label>
			...
			<p><label for="<?php echo $this->get_field_id('tab_one_content'); ?>"><strong><?php _e('Tab 2:', 'dynamicnews') ?></strong></label>
				...
				<label for="<?php echo $this->get_field_id('tab_two_title'); ?>"><?php _e('Title:', 'dynamicnews'); ?></label>
			...
			<p><label for="<?php echo $this->get_field_id('tab_one_content'); ?>"><strong><?php _e('Tab 3:', 'dynamicnews') ?></strong></label>
				...
				<label for="<?php echo $this->get_field_id('tab_three_title'); ?>"><?php _e('Title:', 'dynamicnews'); ?></label>
			...
			<p><label for="<?php echo $this->get_field_id('tab_one_content'); ?>"><strong><?php _e('Tab 4:', 'dynamicnews') ?></strong></label>
				...
				<label for="<?php echo $this->get_field_id('tab_four_title'); ?>"><?php _e('Title:', 'dynamicnews'); ?></label>
		...
		<strong><?php _e('Settings for Recent/Popular Posts and Recent Comments', 'dynamicnews'); ?></strong>
		<p><label for="<?php echo $this->get_field_id('number'); ?>"><?php _e('Number of entries:', 'dynamicnews'); ?></label>
		...
		<label for="<?php echo $this->get_field_id('thumbnails'); ?>"><?php _e('Show Thumbnails?', 'dynamicnews'); ?></label></p>
<?php
	}
}
register_widget('Dynamic_News_Tabbed_Content_Widget');
?>